...
void print_json_schema_start(int tbl)
{
	...
	int fStatus;
	sprintf(baseDirPath, "%s%c%s",
			get_str("DIR"),
			PATH_SEP,
			"schemas");

	if (SCHEMA_FOLDER_CREATED < 1)
	{
		if (bIsVerbose)
		{
			printf("\n    Checking for schemas directory %s...", baseDirPath);
		}

		fStatus = mkdir(baseDirPath, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
		...
		SCHEMA_FOLDER_CREATED = 1;
	}

	sprintf(filepath, "%s%c%s%s",
			baseDirPath,
			PATH_SEP,
	...
	strcat(writebuf, "\", \"columns\" : {");

	/* save the open file to the table struct */
	pTdef->schemaOutfile = fopen(filepath, "w+");
	fpSchemaOutfile = pTdef->schemaOutfile;
...
void print_json_schema_end(int tbl, char *colName, char *colType)
{
	...
	/* fill the write buffer */
	strcpy(writebuf, "\"");
	strcat(writebuf, colName);
	strcat(writebuf, "\":\"");
	strcat(writebuf, colType);
	strcat(writebuf, "\"}}\n");
